...
class TrainerClassification:
    ...
    def run_exp(self, config):
        ...
        if config.extra['step'] != 0:
            self.run_loop(0, config.extra['step'], config)
        for g in self.optim.param_groups:
            g['lr'] = config.extra['lr_better']
        self.run_loop(config.extra['step'], config.extra['step'] + config.epoch_end_at, config)
